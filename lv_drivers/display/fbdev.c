...
void argb8888_to_rgb888(uint32_t argb, uint8_t* r, uint8_t* g, uint8_t* b) {
    *r = (argb >> 16) & 0xFF;  // 获取红色分量
    *g = (argb >> 8) & 0xFF;   // 获取绿色分量
    *b = argb & 0xFF;          // 获取蓝色分量
}
/**
...
void fbdev_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    /*32 or 24 bit per pixel*/
     ...
     /*if(vinfo.bits_per_pixel == 24){//||vinfo.bits_per_pixel == 24) {
       uint32_t * fbp32 = (uint32_t *)fbp;
       int32_t y;
       printf("\r\ny1:%d,y2:%d\r\n",act_y1,act_y2);
       for(y = act_y1; y <= act_y2; y++) {
         // if(act_x1>0){
	//  	location = (act_x1 + vinfo.xoffset-240) + (y + vinfo.yoffset) * finfo.line_length / 4;
	//  }else{
	  	location = act_x1+(y + vinfo.yoffset) * 320;
	//  }
          printf("x1:%d,x2:%d,*4:%d,location:%d,xoffset:%d,yoffset:%d,y:%d,finfo.line_length:%d\r\n",act_x1,act_x2,(act_x2-act_x1 +1)*4,location,vinfo.xoffset,vinfo.yoffset,y,finfo.line_length);
          memcpy(&fbp32[location], (uint32_t *)color_p, (w*3));
          color_p += w;
       }
     }*/
    ...
    if(vinfo.bits_per_pixel == 32 || vinfo.bits_per_pixel == 24) {
       ...
       uint8_t r, g, b;
       for(y = act_y1; y <= act_y2; y++) {
          location = (act_x1*3/4 + vinfo.xoffset) + (y + vinfo.yoffset) * finfo.line_length / 4;
          uint32_t buff[w];//ARGB
          memcpy(&buff[0], (uint32_t *)color_p, (act_x2 - act_x1 + 1) * 4);
          uint8_t  buff_888[w*3];
          for (size_t i = 0; i < w; i++) {
              argb8888_to_rgb888(buff[i], &r, &g, &b);
              buff_888[i * 3] = ~r;      // 存储红色分量
              buff_888[i * 3 + 1] = ~g;  // 存储绿色分量
              buff_888[i * 3 + 2] = ~b;  // 存储蓝色分量
          }
          memcpy((uint8_t *)&fbp32[location]+act_x1*3%4, (uint32_t *)buff_888, w* 3);
          color_p += w;
       }
     }
