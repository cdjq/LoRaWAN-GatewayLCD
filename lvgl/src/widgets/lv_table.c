    ...
    for(row = 0; row < table->row_cnt; row++) {
        ...
        for(col = 0; col < table->col_cnt; col++) {
            ...
            if(table->cell_data[cell]) {
                ...
                lv_area_t img_area;
                img_area.x1 = cell_area.x1 + cell_left/2;
                img_area.x2 = cell_area.x2 - cell_right/2;
                img_area.y1 = cell_area.y1 + cell_top/2;
                img_area.y2 = cell_area.y2 - cell_bottom/2;
                ...
                if(label_mask_ok) {
                    ...
                    //-----绘制单元格图片信息-----
                    if(!strncmp(table->cell_data[cell] + 1, "  ", 3)){
                        LV_IMG_DECLARE(JoinRequest);
                        lv_img_header_t header;
                        lv_res_t res = lv_img_decoder_get_info(&JoinRequest, &header);

                            lv_area_t a;
                            a.x1 = draw_ctx->clip_area->x1 + (lv_area_get_width(draw_ctx->clip_area) - header.w) / 2;
                            a.x2 = a.x1 + header.w - 1;
                            a.y1 = draw_ctx->clip_area->y1 + (lv_area_get_height(draw_ctx->clip_area) - header.h) / 2;
                            a.y2 = a.y1 + header.h - 1;

                        lv_draw_img_dsc_t img_draw_dsc;
                        lv_draw_img_dsc_init(&img_draw_dsc);
                        img_draw_dsc.recolor = lv_color_black();
                        img_draw_dsc.zoom = 256;
                        lv_draw_img(draw_ctx, &img_draw_dsc, &a, &JoinRequest);
                    }else if(!strncmp(table->cell_data[cell] + 1, "   ", 4)){
                        LV_IMG_DECLARE(JoinAccept);
                        lv_img_header_t header;
                        lv_res_t res = lv_img_decoder_get_info(&JoinAccept, &header);
                        ...
                        img_draw_dsc.recolor = lv_color_black();
                        img_draw_dsc.zoom = 256;
                        lv_draw_img(draw_ctx, &img_draw_dsc, &a, &JoinAccept);
                    }else if(!strncmp(table->cell_data[cell] + 1, "    ", 5)){
                        LV_IMG_DECLARE(ConfirmedUplink);
                        lv_img_header_t header;
                        lv_res_t res = lv_img_decoder_get_info(&ConfirmedUplink, &header);
                        ...
                        img_draw_dsc.recolor = lv_color_black();
                        img_draw_dsc.zoom = 256;
                        lv_draw_img(draw_ctx, &img_draw_dsc, &a, &ConfirmedUplink);
                    }else if(!strncmp(table->cell_data[cell] + 1, "     ", 6)){
                        LV_IMG_DECLARE(ConfirmedDownlink);
                        lv_img_header_t header;
                        lv_res_t res = lv_img_decoder_get_info(&ConfirmedDownlink, &header);
                        ...
                        img_draw_dsc.recolor = lv_color_black();
                        img_draw_dsc.zoom = 256;
                        lv_draw_img(draw_ctx, &img_draw_dsc, &a, &ConfirmedDownlink);
                    }else if(!strncmp(table->cell_data[cell] + 1, "      ", 7)){
                        LV_IMG_DECLARE(UnconfirmedUplink);
                        lv_img_header_t header;
                        lv_res_t res = lv_img_decoder_get_info(&UnconfirmedUplink, &header);
                        ...
                        img_draw_dsc.recolor = lv_color_black();
                        img_draw_dsc.zoom = 256;
                        lv_draw_img(draw_ctx, &img_draw_dsc, &a, &UnconfirmedUplink);
                    }else if(!strncmp(table->cell_data[cell] + 1, "       ", 8)){
                        LV_IMG_DECLARE(UnconfirmedDownlink);
                        lv_img_header_t header;
                        lv_res_t res = lv_img_decoder_get_info(&UnconfirmedDownlink, &header);
                        ...
                        img_draw_dsc.recolor = lv_color_black();
                        img_draw_dsc.zoom = 256;
                        lv_draw_img(draw_ctx, &img_draw_dsc, &a, &UnconfirmedDownlink);
                    }
